...
- (void)setTimeEndBlock:(void (^)(void))axcUI_buttonTimeStoppedCallback {
...
- (void)AxcTool_countDownWithTimeInterval:(NSTimeInterval)duration countDownFormat:(NSString *)format{
    if (!format){
...
- (void)AxcTool_cancelTimer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.timeEndBlock) { self.timeEndBlock(); }
    });
}
