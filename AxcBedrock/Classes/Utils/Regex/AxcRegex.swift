...
/// 正则匹配工具
public typealias AxcRegex = AxcBedrockLib.Regex

// MARK: AxcRegex.Options

/// 正则匹配工具
public extension AxcRegex {
    ...
    struct Regex {
        ...
        public func replacingMatches(in input: String,
            ...
            for match in rangedMatches.reversed() {
                ...
                output = output.replacingOccurrences(of: match.string, with: replacement)
            }
...
public extension AxcRegex {
    ...
    class Match: CustomStringConvertible {
        /// 初始化
        internal init(result: NSTextCheckingResult,
                      in string: String) {
            precondition(result.regularExpression != nil,
        ...
        public lazy var range: Range<String.Index> = {
            return Range(self.result.range,
                         in: self.baseString)!
        }()
        ...
        public func string(applyingTemplate template: String) -> String {
            let replacement = result.regularExpression!.replacementString(for: result,
                                                                          in: baseString,
                                                                          offset: 0,
                                                                          template: template)
            return replacement
